<html>  
    ...
    <body>  
		<div class="panel-body" style="padding-bottom:0px;">
	        <div class="panel panel-default">
	            <div class="panel-heading">查询条件<em>数据库</em></div>
	            <div class="panel-body">
	                <form id="formSearch" class="form-horizontal">
	                    <div class="form-group" style="margin-top:15px">
	                        <label class="control-label col-sm-1" for="txt_search_departmentname">表名称</label>
	                        <div class="col-sm-3">
	                            <input type="text" class="form-control" id="txt_search_departmentname">
	                        </div>
	                        <label class="control-label col-sm-1" for="txt_search_statu">查找</label>
	                        <div class="col-sm-3">
	        ...
	        <div id="toolbar" class="btn-group">
	            ...
	            <button id="btn_edit" type="button" class="btn btn-default btn-primary">
	        ...
	        <div class='row'>
	        	<div id="menuContent" class="menuContent col-sm-2" style="height:calc(100% - 235px);border:1px solid rgb(170,170,170);z-index:10;overflow-y:auto;">
				  <ul id="treeDemo" class="ztree" style="margin-top:0; width:100%; height:auto;"></ul>
				</div>
				<div class='col-sm-10' style='height:calc(100% - 290px);'>
				<table id="tb_departments"></table>
				</div>
	        </div>
	    </div>
	    <form id="freshen" action="${CONTEXTPATH }/mysql/host" method="get">
	    	<input type='hidden' name="p" vlaue="1" id="p" />
	    </form>
    </body>
    <script type="text/javascript">
       ...
       <#if zNodes??>
            <#list zNodes as item>
            	<#assign  keys=item?keys/>
            	var map = {id:"${item.id}",name:"${item.name}"};
            	zNodes.push(map);
            </#list>
       </#if>
    $(function () {
	...
	function onClick(event, treeId, treeNode){
		...
		freshenPage(treeNode['id']);
	}
	function freshenPage(id){
		$("#p").val(id);
		$("#freshen").submit();
	}
    </script>

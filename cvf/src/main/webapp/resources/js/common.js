...
// 判断输入的数字不大于某个特定的数字
function MaxValue(str,n) {
	if (str.length != 0) {
		reg = /^[-+]?\d*$/;
		if (reg.test(str)) {// 判断是否为数字类型
			if (val > parseInt(n)) // “123”为自己设定的最大值
			{
				return true;
			}
		}
...
function docWriter() {
	...
	var arr = [];
	arr.push(year + "年", (today.getMonth() + 1) + "月", today.getDate() + "日  "
			+ d[today.getDay() + 1]);
...
function isProjectNbr(username, min, max) {
	// var username=$.trim($(ob).val());
	var flag = false;
	var pattern1 = new RegExp("^[a-zA-Z0-9-_][a-zA-Z0-9-_]{" + min + "," + max
			+ "}$", "gi");
	if (pattern1.test(username) == true) {
		flag = true;
	}
...
function isEmail(email) {
	...
	if (email
			.search(/^[A-Za-z0-9]+([-_.][A-Za-z0-9]+)*@([A-Za-z0-9]+[-.])+[A-Za-z0-9]{2,5}$/) == -1) {
	} else {
		flag = true;
	}
	return flag;
...
function isPhone(phone) {
	...
	if (phone
			.search(/((\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)/) == -1) {
	} else {
		flag = true;
	}
	return flag;
...
function checkConfirmPassword(confirmPassword, min, max, pwd) {
	// var pwd=$("#"+pwdID).val();
	var flag = false;
	if (confirmPassword.length >= min && confirmPassword.length <= max
			&& confirmPassword != '' && confirmPassword == $.trim(pwd)) {
		flag = true;
	}
	return flag;
}
...
function lxfEndtime() {
	$(".lxftime").each(
			function() {
				...
				if (endtime <= nowtime) {
					$(this).html(
							new Date($(this).attr("endtime"))
									.format("yyyy-MM-dd hh:mm:ss")
									+ "（已过期）")// 如果结束日期小于当前日期就提示过期啦
				} else {
					if ($(this).attr("lxfday") == "no") {
						$(this).html(
								"<i>剩余时间：</i><span>" + CHour + "</span>时<span>"
										+ CMinute + "</span>分<span>" + CSecond
										+ "</span>秒"); // 输出没有天数的数据
					} else {
						$(this).html(
								"<i>剩余时间：</i><span>" + days
										+ "</span><em>天</em><span>" + CHour
										+ "</span><em>时</em><span>" + CMinute
										+ "</span><em>分</em><span>" + CSecond
										+ "</span><em>秒</em>"); // 输出有天数的数据
					}
				}
			});
 ...
 * Mobile : /^((\(\d{2,3}\))|(\d{3}\-))?13\d{9}$/ Url :
 * /^http:\/\/[A-Za-z0-9]+\.[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"\"])*$/
 * IdCard : /^\d{15}(\d{2}[A-Za-z0-9])?$/ QQ : /^[1-9]\d{4,8}$/
 * 某种特殊金额：/^((\d{1,3}(,\d{3})*)|(\d+))(\.\d{2})?$/ //说明：除“XXX XX,XXX
 * XX,XXX.00”格式外
 * 
 * //为上面提供各个JS验证方法提供.trim()属性 String.prototype.trim=function(){ return
 * this.replace(/(^\s*)|(\s*$)/g, ""); }
 * 
 * 调用： <input type="text" name="str" > <input type="button" value=" 确定 "
 * onClick=""> //onClick中写自己要调用的JS验证函数
 * 
 * <script language="javascript" type="text/javascript"> var patterms = new
 * Object(); //验证IP patterms.ip =
 * /^(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])(\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])){3}$/;
 * //验证EMAIL patterms.email =
 * /^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/; //验证日期格式2009-07-13
 * patterms.date = /^\d{4}-(0?[1-9]|1[0-2])-(0?[1-9]|[1-2]\d|3[0-1])$/;
 * //验证时间格式16:55:39 patterms.time = new
 * RegExp("^([0-1]\\d|2[0-3]):[0-5]\\d:[0-5]\\d$"); //验证函数 function
 * verify(str,pat) { var thePat; thePat = patterms[pat]; if(thePat.test(str)) {
 * return true; } else { return false; } } //测试
 ...
 * 
 * 
 * afterpaste(ob) ：只能输入数字(ob:对象) keyUp(ob) ：只能输入数字(ob:对象) boolean
 * isUsername(username,min,max) ：验证用户名 字母开头字母或数字结尾 username:用户名 min:
 * 最少几位，max：最多几位 boolean isEmail(email) ：验证邮箱 boolean isMobile(mobile) ：验证手机号码
 * boolean isPhone(phone) ：验证固话号码 boolean checkQQ(QQNum) ：验证QQ号码 boolean
 * isPostalCode(postalCode) ：验证邮政编码 boolean isCardNo(card) ：验证身份证号码 boolean
 * checkIdcard(card) ：验证身份证号码（验证出生日期和校验位） boolean isURL(url) ：验证是否是URL boolean
 * isIp(ip) ：验证IP boolean checkPassword(password,min,max) ：验证密码长度 boolean
 * checkConfirmPassword(confirmPassword,min,max,pwd) ：验证确认密码长度 pwdID第一个密码
 * boolean isNull(str) ：检查输入字符串是否为空或者全部都是空格 boolean isLastMatch(str1,str2) :
 * 检查字符1是否以字符串2结束 boolean isFirstMatch(str1,str2) : 检查字符1是否以字符串2开始 boolean
 * isMatch(str1,str2) : 检查字符1是包含字符串2 boolean ischinese(str) : 检查输入是否为中文的函数
 * boolean isNumber(numStr) : 判断一個字符串是否是数值格式 boolean isPositiveInteger(str) :
 * 判断是否是正整數 boolean isValidString(szStr) ：判断字符串中是否含有非法字符"'\"><@#$%^&*~!+()。，,.‘、[]`;|"
 * boolean isIncSym(szStr) ：是否包含系统禁用的字符 boolean isBlank(szStr)
 * ：判断字符串是否爲空字符串（或不包含除空格外的其他字符） String trim(szStr) ：去掉字符串兩端的空格 boolean
 * isDateStr(ds) ：判断字符串是否爲合法的日期格式：YYYY-MM-DD HH:MM:SS 或 YYYY-MM-DD 或 HH:MM:SS
 * boolean isDatePart(dateStr) ：判断字符串是否爲合法的日期格式：YYYY-MM-DD boolean
 * isTimePart(dateStr) ：判断字符串是否爲合法的時間格式：HH:MM:SS int strlen(str) :
 * 精确判断字符串長度，包括中文和英文混合 boolean isKong（szStr） ：判断字符串中是否含有空格（去掉字符串前後空格後） boolean
 * isLenBetween(szStr,minl,maxl) ：用户输入字符串长度是否在两值之间 boolean isNumberOrLetter(str)
 * ：判断输入字符串是否只由英文字母和数字组成 boolean isChinaOrNumbOrLett(str) ：判断输入字符串是否只由汉字、字母、数字组成
 * String getMaxDay(year,month) ：返回某年某月的最大天数 int checkSelectNum( checkboxID )
 * ：检查复选框被选中的数目 checkboxID ：复选框id String checkBrowser() : 验证是什么浏览器 String
 * checkIE() ：验证浏览器是IE6，还是IE7 boolean isLetterNumHyphen(value) ：验证是否由英文、数字、-、_組成
 * boolean checkSelect(formObj, eleName) ：验证是否有选择复选框，（formObj: form对象
 * eleName：复选框name） selectOrNotAll(formObj, eleName, selectOrNotSelect)
 * ：全选或取消选择复选框 （formObj: form对象 eleName：复选框name selectOrNotSelect：true或false）
 * reverseSelect(formObj, eleName) ：反择复选框，（formObj: form对象 eleName：复选框name）
 * boolean int getTextareaStrLength(str) ：验证输入框/文本框 字节数。。
 ******************************************************************************/

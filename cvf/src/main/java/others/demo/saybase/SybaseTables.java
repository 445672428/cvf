...
public class SybaseTables {
    ...
    public static void create() throws Exception {
        ...
        String keySql =
                " select   indid, keycnt, name,object_name(id) as tabname from sysindexes where  indid >=1 and status&2048=2048 ";// --
                                                                                                                                  // indid大于1
                                                                                                                                  // 表示主键或索引
        ...
        for(String table : taleNames) {
            String tableInfo =
                    "SELECT sysobjects.name AS tablename ,syscolumns.name as colname,syscolumns.length as colLength,systypes.name as colType,systypes.variable,systypes.allownulls "
    ...
    public static void name1() throws Exception {
        ...
        table2.addCell(new Paragraph("用java生成的表格1"));
        table2.addCell(new Paragraph("用java生成的表格2"));
        table2.addCell(new Paragraph("用java生成的表格3"));
        table2.addCell(new Paragraph("用java生成的表格4"));
        document.add(table2);
